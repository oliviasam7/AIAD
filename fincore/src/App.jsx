...
import { analyzeContract, readFileAsText, extractTextFromPDF } from "./utils/api";
...
function AppInner() {
  ...
  const handleAnalyze = async () => {
    ...
    try {
      ...
      if (tab === "text") {
        ...
        if (file.type.startsWith("image/")) {
          ...
          if (!extractedText || extractedText.trim().length < 40) {
            throw new Error("Could not extract text from this PDF. It may be scanned or image-based. Try uploading as an image instead.");
          }
      ...
      // Store the real extracted text for the chatbot
      setContractText(extractedText || analysis._contractText || "");
      setResult(analysis);
      setStatus("done");
